      ...
      `;
        ...
        const displayOptions = () => {
            ...
            for (let value in options) {
                buttonCon.innerHTML += `<button class="option-button" onclick="generateWord('${value}')">${value}</button>`;
            }
        ...
        // Fix for the checkLetter function
        const checkLetter = (button) => {
            let charArray = chosenWord.toLowerCase().split("");
            let buttonLetter = button.innerText.toLowerCase();
            let dashes = document.getElementsByClassName("dashes");

            // Check if the word contains the letter (case insensitive)
            if (charArray.includes(buttonLetter)) {
                // Find all occurrences of the letter (preserving original case)
                chosenWord.split("").forEach((char, index) => {
                    if (char.toLowerCase() === buttonLetter) {
                        // Replace dash with the original case letter from chosenWord
                        dashes[index].innerText = chosenWord[index];
                        // Animate the revealed letter
                        animateLetterReveal(dashes[index]);
                        // Increment counter
                        winCount += 1;
                    }
                ...
                if (winCount === chosenWord.length) {
                    resultText.innerHTML = `<h2 class='win-msg'>You Win!!</h2><p>The word was <span>${chosenWord}</span><br>Time: ${document.getElementById('timer').textContent}</p>`;
                    updateScores('win', chosenWord);
                    triggerWinAnimation();
                    blocker();
                }
        ...
        const initializer = () => {
            ...
            // First try to load a saved game
            if (!loadGameProgress()) {
        ...
        const drawMan = (count) => {
            ...
            switch (count) {
                ...
                case 6:
                    ...
                    break;
                case 7:
                    // Only in easy mode - draw additional parts
                    context = canvas.getContext("2d");
                    context.beginPath();
                    context.strokeStyle = "#000";
                    context.lineWidth = 1;
                    // Draw face details
                    context.arc(67, 28, 2, 0, Math.PI * 2); // left eye
                    context.stroke();
                    context.beginPath();
                    context.arc(73, 28, 2, 0, Math.PI * 2); // right eye
                    context.stroke();
                    context.beginPath();
                    context.arc(70, 32, 3, 0, Math.PI, false); // mouth
                    context.stroke();
                    break;
                case 8:
                    // Only in easy mode - final part
                    context = canvas.getContext("2d");
                    context.beginPath();
                    context.strokeStyle = "#000";
                    context.lineWidth = 1;
                    // Draw hat
                    drawLine(60, 20, 80, 20);
                    drawLine(63, 16, 77, 16);
                    drawLine(63, 16, 63, 20);
                    drawLine(77, 16, 77, 20);
                    break;
                default:
                    break;
            }
        ...
        document.addEventListener('keydown', (event) => {
            ...
            if (letterContainer.classList.contains('hide') || newGameContainer.classList.contains('hide') === false) {
                return;
            }
